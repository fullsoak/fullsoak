 ...
 */
export const byoHtml = (component: VNode) => "<!doctype html>" + render(component);

/**
 * takes a standard Preact component and wrap it around an <html> element
 * and render it out as a string
 */
export const ssr = async (component: FunctionComponent) => {
  ...
  try {
    ...
    componentCss = await Deno.readTextFile(`${globalThis.FULLSOAK_APP_COMPONENTS_DIR}/${componentName}/styles.css`);
  } catch (e) {
    console.warn(`unable to load css for component ${componentName}: ${(e as Error).message}`);
  }
